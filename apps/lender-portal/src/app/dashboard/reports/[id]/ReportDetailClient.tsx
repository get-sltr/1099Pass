...
const mockReport = {
  ...
  incomeSources: [
    { name: 'Uber', monthlyAvg: 3200, yearlyTotal: 38400, percentage: 53, trend: 'growing', verified: true, months: 24 },
    { name: 'DoorDash', monthlyAvg: 1800, yearlyTotal: 21600, percentage: 30, trend: 'stable', verified: true, months: 18 },
    { name: 'Freelance Web Dev', monthlyAvg: 1042, yearlyTotal: 12500, percentage: 17, trend: 'growing', verified: true, months: 12 },
  ],
...
export function ReportDetailClient({ reportId }: { reportId: string }) {
  ...
  const inShortlist = isInShortlist(reportId);

  const toggleShortlist = () => {
    if (inShortlist) removeFromShortlist(reportId);
    else addToShortlist(reportId);
  };
          ...
          <Dialog open={showContactDialog} onOpenChange={setShowContactDialog}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Send a message expressing interest in this borrower.
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="h-[250px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={mockReport.monthlyHistory}>
                      ...
                      <Line type="monotone" dataKey="income" stroke="#1B4D3E" strokeWidth={2} dot={{ fill: '#1B4D3E' }} name="Actual" />
                      <Line type="monotone" dataKey="projected" stroke="#94A3B8" strokeDasharray="5 5" strokeWidth={2} dot={false} name="Projected" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
                ...
                <ul className="space-y-4">
                  {mockReport.riskFactors.map((risk, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <Badge variant={risk.level === 'low' ? 'success' : risk.level === 'medium' ? 'warning' : 'error'} className="mt-0.5 shrink-0">
                        {risk.level}
                      </Badge>
                      <div>
                      ...
                      <tr key={source.name}>
                        ...
                        <td>
                          <div className="flex items-center gap-1">
                            {source.trend === 'growing' ? <TrendingUp className="h-4 w-4 text-positive" /> : source.trend === 'declining' ? <TrendingDown className="h-4 w-4 text-negative" /> : <span className="h-4 w-4 flex items-center justify-center text-muted-foreground">—</span>}
                            <span className="text-sm capitalize">{source.trend}</span>
                          </div>
                        </td>
                        <td>
                          {source.verified ? <Badge variant="success">Verified</Badge> : <Badge variant="warning">Pending</Badge>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <Card>
              ...
              <CardContent>
                <div className="h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={mockReport.incomeSources.map((s) => ({ name: s.name, value: s.percentage }))} cx="50%" cy="50%" innerRadius={40} outerRadius={70} paddingAngle={2} dataKey="value">
                ...
                <div className="space-y-2 mt-4">
                  {mockReport.incomeSources.map((source, i) => (
                    <div key={source.name} className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-muted-foreground ml-auto">{source.percentage}%</span>
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
                  ...
                  {mockReport.verificationDetails.bankConnected ? <CheckCircle className="h-5 w-5 text-positive" /> : <AlertCircle className="h-5 w-5 text-negative" />}
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
                  <div className="flex items-center gap-3">
                  ...
                  {mockReport.verificationDetails.taxReturnsVerified ? <CheckCircle className="h-5 w-5 text-positive" /> : <AlertCircle className="h-5 w-5 text-negative" />}
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
                  <div className="flex items-center gap-3">
                    <Briefcase className="h-5 w-5 text-accent" />
                  ...
                  <span className="font-mono font-medium">{mockReport.verificationDetails.platformsLinked}</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
                  <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm">{mockReport.verificationDetails.lastVerification}</span>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="text-center py-6">
                  <div className="inline-flex items-center justify-center h-32 w-32 rounded-full bg-accent/10">
                    <span className="text-4xl font-bold text-accent">{mockReport.verificationDetails.verificationScore}%</span>
                  </div>
                  <p className="mt-4 text-muted-foreground">
                    All income sources have been independently verified through bank connections and platform integrations.
                  </p>
                </div>
              </CardContent>
            </Card>
                  ...
                  {mockReport.riskFactors.map((risk, i) => (
                    <li key={i} className="p-4 rounded-lg border bg-card">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge variant={risk.level === 'low' ? 'success' : risk.level === 'medium' ? 'warning' : 'error'}>{risk.level} risk</Badge>
                      </div>
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-lg bg-positive/10 border border-positive/20">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium text-positive">Recommended for Approval</span>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      This borrower has a strong income score with diversified sources and verified documentation.
                    </p>
                  </div>
                  <div className="space-y-2">
